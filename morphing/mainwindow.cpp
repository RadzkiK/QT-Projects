...
MainWindow::MainWindow(QWidget *parent)
    ...
    morphed_img(500, 500, QImage::Format_RGB32),
    ui(new Ui::MainWindow)
{
    ...
    img_tmp = img;
    img2.load("C://Users/Konrad/Pictures/sponge.png");
    img2_tmp = img2;
}
...
void MainWindow::paintEvent(QPaintEvent *)
{
    ...
    if(img_choice == 0) { p.drawImage(0,0,img); }
    if(img_choice == 1) { p.drawImage(0,0,img2); }
    p.drawImage(510,0,morphed_img);
    update();
}
...
void MainWindow::mousePressEvent(QMouseEvent *e)
{
    ...
    if(e->button() == Qt::RightButton)
    {
        ...
        if(img_choice == 0 ) img.load("C://Users/Konrad/Pictures/earth.jpg");
        if(img_choice == 1 ) img2.load("C://Users/Konrad/Pictures/sponge.png");
    }
    else if(e->button() == Qt::MiddleButton)
    {
        ...
        } else if(img_choice == 1)
        {
            if(!points2.empty())
            {
                ...
                {
                    if(sqrt(pow(position.x() - (points2[it].x()), 2) + pow(y0 - points2[it].y(), 2)) <= 2)
                    {
                        iterat = it;
                        copy = points2;
                    }
...
//    }
    ...
    {
        if(position.x() >= 0 && position.x() < 500 && position.y() >= 0 && position.y() < 500 && img_choice == 0)
        {
        ...
        } else if(position.x() >= 0 && position.x() < 500 && position.y() >= 0 && position.y() < 500 && img_choice == 1)
        {
...
{
    if(p.x() >= 0 && p.x() < 500 && p.y() >= 0 && p.y() < 500 && img_choice == 0)
    {
        if(points.size() < 3)
        {
            points.push_back(p);
    ...
    } else if(p.x() >= 0 && p.x() < 500 && p.y() >= 0 && p.y() < 500 && img_choice == 1)
    {
        if(points2.size() < 3)
        {
            points2.push_back(p);
...
void MainWindow::morph(int i)
{
    morphed_img.fill(0);
    float i1 = 1.-(float)i/(float)n;
    float i2 = (float)i/(float)n;
    int x_a = i1*points[0].x() + i2*points2[0].x();
    int x_b = i1*points[1].x() + i2*points2[1].x();
    int x_c = i1*points[2].x() + i2*points2[2].x();
    int y_a = i1*points[0].y() + i2*points2[0].y();
    int y_b = i1*points[1].y() + i2*points2[1].y();
    int y_c = i1*points[2].y() + i2*points2[2].y();
    ...
    for(int y = 0; y < 500; ++y)
    {
        for(int x = 0; x < 500; ++x)
        {
            ...
            if(u > 0 && u < 1 && v > 0 && v < 1 && w > 0 && w < 1)
            {
                ...
                x2_t = u * (float)points2[0].x() + v * (float)points2[1].x() + w * (float)points2[2].x();
                y2_t = u * (float)points2[0].y() + v * (float)points2[1].y() + w * (float)points2[2].y();



                im_c = img.pixel((int)x_t,(int)y_t);
                im2_c = img2.pixel((int)x2_t,(int)y2_t);
                out.setRed(i1*im_c.red() + i2*im2_c.red());
                out.setGreen(i1*im_c.green() + i2*im2_c.green());
                out.setBlue(i1*im_c.blue() + i2*im2_c.blue());

                morphed_img.setPixelColor(x,y,out);
            }
        }
    }
}
...
void MainWindow::on_frames_slider_valueChanged(int value)
{
    frame = value;
    morph(frame);
    update();
...
void MainWindow::on_img_choice_slider_valueChanged(int value)
{
    img_choice = value;
    update();
}
